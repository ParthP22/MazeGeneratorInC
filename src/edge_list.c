...
EdgeList* edgelist_init(int capacity){
    ...
    edge_list->arr = (int(*)[4])malloc(sizeof(int)* 4 * capacity);
    edge_list->capacity = capacity;
    edge_list->size = 0;
    return edge_list;
}

bool edgelist_add(EdgeList* edge_list, int edge[4]){
    if(edge_list == NULL){
        printf("\nEdgeList is null");
        return false;
    }
...
bool edgelist_insert(EdgeList* edge_list, int index, int edge[4]){
    if(edge_list == NULL){
        printf("\nEdgeList is null");
        return false;
    }
...
int* edgelist_set(EdgeList* edge_list, int index, int edge[4]){
    if(edge_list == NULL){
        printf("\nEdgeList is null");
        return NULL;
    }
...
void edgelist_to_string(EdgeList* edge_list){
    ...
    printf("[%d, %d, %d, %d]]", edge_list->arr[edge_list->size - 1][0],
...
int (*_edgelist_reallocate(EdgeList* edge_list))[4]{
    int new_cap =  edge_list->capacity * 2;
    int (*new_arr)[4] = (int(*)[4])malloc(sizeof(int) * new_cap * 4);
